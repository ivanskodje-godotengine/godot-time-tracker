...
func _enter_tree():
	...
	# Add signal to all 42 buttons, which returns button node
	for i in range(42):
...
func check_position():
	...
	popup.set_global_pos(get_global_pos())
	...
	var x_total = cal.get_global_pos().x + cal.get_size().x + popup_container.get_pos().x + popup_container.get_size().y
	...
	var y_total = cal.get_global_pos().y + cal.get_size().y + popup_container.get_pos().y + popup_container.get_size().y
...
func toggled(is_pressed):
	if(!is_pressed):
		...
		if(get_tree().get_root().has_node("popup")):
			popup.show()
		else:
